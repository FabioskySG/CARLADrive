...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def generate_spawn_points():
        ...
        wps = local_map.generate_waypoints(5.0)  # Waypoints dispersos a lo largo de la carretera
    ...
    def request_new_batch_actors(model, amount, spawn_points, autopilot=False,
        ...
        # Inicializar variables de control de tiempo si no existen
        # if not hasattr(CarlaDataProvider, '_pedestrian_last_cleanup_time'):
        #     CarlaDataProvider._pedestrian_last_cleanup_time = 0.0
        #     CarlaDataProvider._spawned_pedestrians = []
        #     CarlaDataProvider._pedestrian_cleanup_interval = 20.0  # 20 segundos

        # # Obtener el tiempo actual de simulación
        # current_time = CarlaDataProvider._world.get_snapshot().timestamp.elapsed_seconds
        
        # # Verificar si han pasado 20 segundos desde la última limpieza
        # time_since_cleanup = current_time - CarlaDataProvider._pedestrian_last_cleanup_time

        # if time_since_cleanup >= CarlaDataProvider._pedestrian_cleanup_interval:
        #     CarlaDataProvider._remove_pedestrians_by_time(current_time)
        #     CarlaDataProvider._pedestrian_last_cleanup_time = current_time
        ...
        #     # Agregar los nuevos peatones con timestamp
        #     for walker in walkers:
        #         CarlaDataProvider._spawned_pedestrians.append(walker)
    ...
    @staticmethod
    def spawn_pedestrians_on_sidewalks(pedestrians_removed = False):
        """
        Spawn pedestrians randomly on precomputed sidewalk waypoints.
        """

        if not pedestrians_removed:
            return

        CarlaDataProvider.generate_spawn_points()

        if not CarlaDataProvider._sidewalk_wps:
            return
        ...
        random_wps = np.random.choice(CarlaDataProvider._sidewalk_wps, size=10, replace=False)
    ...
    @staticmethod
    def update_pedestrian_cleanup():
        """Función que debe ser llamada regularmente para limpiar peatones"""
        if not hasattr(CarlaDataProvider, '_pedestrian_last_cleanup_time'):
            CarlaDataProvider._pedestrian_last_cleanup_time = 0.0
            CarlaDataProvider._spawned_pedestrians = []
            CarlaDataProvider._pedestrian_cleanup_interval = 30.0
    ...
    # Método para eliminar peatones basado en tiempo (agregar a CarlaDataProvider)
    @staticmethod
    def _remove_pedestrians_by_time(current_time):
        """Elimina todos los peatones spawneados"""
        if not hasattr(CarlaDataProvider, '_spawned_pedestrians'):
            return
        
        removed_count = 0
        pedestrians_to_keep = []
        
        print(f"Time {current_time:.2f}s: Removing pedestrians...")
        
        for pedestrian in CarlaDataProvider._spawned_pedestrians:
            try:
                if pedestrian.is_alive:
                    # Eliminar del pool de actores
                    if pedestrian.id in CarlaDataProvider._carla_actor_pool:
                        del CarlaDataProvider._carla_actor_pool[pedestrian.id]
                    
                    # Destruir el actor
                    pedestrian.destroy()
                    removed_count += 1
                    print(f"Removed pedestrian ID: {pedestrian.id}")
                else:
                    # El peatón ya fue destruido por otro proceso
                    if pedestrian.id in CarlaDataProvider._carla_actor_pool:
                        del CarlaDataProvider._carla_actor_pool[pedestrian.id]
                    
            except Exception as e:
                print(f"Error removing pedestrian: {e}")
                # Mantener en la lista para intentar eliminarlo después
                pedestrians_to_keep.append(pedestrian)
        
        # Actualizar la lista con los peatones que no se pudieron eliminar
        CarlaDataProvider._spawned_pedestrians = pedestrians_to_keep
        
        print(f"Removed {removed_count} pedestrians at time {current_time:.2f}s. Remaining: {len(CarlaDataProvider._spawned_pedestrians)}")
